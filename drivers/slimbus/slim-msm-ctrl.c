...
static int msm_xfer_msg(struct slim_controller *ctrl, struct slim_msg_txn *txn)
{
	...
	if (mc == SLIM_USR_MC_GENERIC_ACK) {
		...
		pr_debug("-slimdebug-generic ack:0x %x, mgrstat:0x%x", pbuf[0], mgrstat);
	} /* slimbus debug patch */
	mutex_unlock(&dev->tx_lock);
	if (msgv >= 0)
		msm_slim_put_ctrl(dev);
...
static int msm_sat_define_ch(struct msm_slim_sat *sat, u8 *buf, u8 len, u8 mc)
{
	...
	if (mc == SLIM_USR_MC_CHAN_CTRL) {
		...
		ret = slim_control_ch(&sat->satcl, sat->satch[i].chanh, oper,
					false);
		pr_debug("-slimdebug-SAT oper:%d grp start:%d, ret:%d", oper,
		...
		if (mc == SLIM_USR_MC_DEF_ACT_CHAN) {
			...
			pr_debug("-slimdebug-SAT activate grp start: ret:%d", ret); /* slimbus debug patch */
		}
	}
	return ret;
}
...
send_capability:
		...
		case SLIM_USR_MC_RECONFIG_NOW:
			tid = buf[3];
			gen_ack = true;
			pr_debug("-slimdebug-SAT:LA:%x reconf req", sat->satcl.laddr); /* slimbus debug patch */
		...
		case SLIM_USR_MC_CONNECT_SINK:
			...
			txn.wbuf = wbuf;
			gen_ack = true;
			pr_debug("-slimdebug-SAT connect MC:0x%x,LA:0x%x", txn.mc,
					sat->satcl.laddr); /* slimbus debug patch */
			ret = msm_xfer_msg(&dev->ctrl, &txn);
		...
		case SLIM_USR_MC_DISCONNECT_PORT:
			...
			txn.wbuf = wbuf;
			gen_ack = true;
			pr_debug("-slimdebug-SAT disconnect LA:0x%x", sat->satcl.laddr); /* slimbus debug patch */
			ret = msm_xfer_msg(&dev->ctrl, &txn);
		...
		if (!ret)
			...
			pr_debug("-slimdebug-sat cmd:0x%x no ack:%d", mc, ret); /* slimbus debug patch */
			wbuf[1] = 0;
		}
		...
		ret = msm_xfer_msg(&dev->ctrl, &txn);
		if (ret) {
			pr_debug("-slimdebug-sending ACK failed:%d", ret);
